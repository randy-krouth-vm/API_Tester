...
public static class CveCorpusService
{
    ...
    public static async Task<string> BuildSummaryAsync(int sampleCount = 50)
    {
        ...
        await foreach (var record in ReadRecordsAsync())
        {
            ...
            {
                var preview = mapped.Count == 0
                    ? "No mapped test"
                    : string.Join(", ", mapped);
                samples.Add($"- {record.CveId} | {record.Severity} | CWE={record.Cwe} | Maps={preview}");
            }
        }
        ...
        sb.AppendLine();
        sb.AppendLine($"Sample ({sampleCount}):");
    ...
    public static async Task<string> BuildCorpusPageAsync(int page, int pageSize = 250)
    {
        ...
        {
            sb.AppendLine($"- [{row.RowNumber}] {row.CveId} | Sev={row.Severity} | Score={row.Score} | CWE={row.Cwe}");
        }
    ...
    public static async Task<string> BuildFunctionMapPageAsync(int page, int pageSize = 250)
    {
        ...
        sb.AppendLine($"Page: {pageData.Page}/{pageData.TotalPages} | Page size: {pageData.PageSize} | Total mapped CVEs: {pageData.TotalRows}");
        var firstRow = pageData.Rows.Count == 0 ? 0 : pageData.Rows[0].RowNumber;
        var lastRow = pageData.Rows.Count == 0 ? 0 : pageData.Rows[^1].RowNumber;
        sb.AppendLine($"Showing rows {firstRow} to {lastRow}");
        sb.AppendLine();
        foreach (var row in pageData.Rows)
        {
            sb.AppendLine($"- [{row.RowNumber}] {row.CveId} | {row.Confidence} ({row.ConfidenceScore}) | Prevent={row.PreventionScore}/100 | CWE={row.Cwe} | {row.FunctionsPreview}");
        }
    ...
    public static async Task<CveCorpusPageResult> GetCorpusPageAsync(int page, int pageSize = 250)
    {
        ...
        var totalPages = Math.Max(1, (int)Math.Ceiling(meta.Count / (double)pageSize));
        if (page > totalPages)
        {
            page = totalPages;
        }
        ...
        var idx = 0;
        var rows = new List<CveCorpusPageRow>(pageSize);
        ...
        return new CveCorpusPageResult(page, totalPages, pageSize, meta.Count, rows);
    }
    ...
    public static async Task<CveFunctionMapPageResult> GetFunctionMapPageAsync(int page, int pageSize = 250)
    {
        ...
        var end = start + pageSize;
        var idx = 0;
        var rows = new List<CveFunctionMapPageRow>(pageSize);
        ...
        return new CveFunctionMapPageResult(page, totalPages, pageSize, meta.Count, rows);
    }
    ...
    public static async Task<IReadOnlyList<CveFunctionMapPageRow>> GetAllFunctionMapRowsByConfidenceAsync(string confidence)
    {
        ...
        var rows = new List<CveFunctionMapPageRow>();
        var idx = 0;
        ...
        while (true)
        {
            ...
            idx++;
            var record = ParseFunctionMapRecord(line);
            if (record is null || !record.Confidence.Equals(normalized, StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
        ...
        int pageSize = 250)
    {
        ...
        var normalized = confidence.Trim().ToUpperInvariant();
        var lastWriteUtc = File.GetLastWriteTimeUtc(FunctionMapFilePath);
        ...
        if (sorted.Length == 0)
        {
            ...
            lock (FunctionMapFilterCacheLock)
            {
                FunctionMapFilterCache[normalized] = new FunctionMapFilterCacheEntry(lastWriteUtc, sorted);
            }
        }
    ...
    public static async Task<string> BuildCveLookupAsync(string cveId)
    {
        ...
        if (mapRecord is not null)
        {
            ...
            sb.AppendLine($"- Functions: {string.Join(", ", mapRecord.Functions)}");
    ...
    public static async Task<string> BuildFunctionMapSummaryAsync(int sampleCount = 50)
    {
        if (!File.Exists(FunctionMapMetaFilePath) || !File.Exists(FunctionMapFilePath))
        {
        ...
        var samples = new List<CveFunctionMapRecord>();
        double scoreTotal = 0;
        var scoreCount = 0;
        ...
        sb.AppendLine();
        sb.AppendLine($"Sample ({samples.Count}):");
        foreach (var sample in samples)
        {
            var functions = string.Join(", ", sample.Functions);
            sb.AppendLine($"- {sample.CveId} | CWE={sample.Cwe}");
            sb.AppendLine($"  {CveCoverageMapper.ToDetectionLabel(sample.Confidence)} | Confidence score={sample.ConfidenceScore:F2}");
            sb.AppendLine($"  EstimatedDefaultCoverage={sample.RealWorldCoverageScore:F2}/100");
            sb.AppendLine($"  Maps={functions}");
            sb.AppendLine($"  Default settings: {sample.DefaultSettings}");
        }

        if (samples.Count == 0)
        {
            sb.AppendLine("- No mapped records found.");
        }
    ...
    private static string ResolveCacheDirectory()
    {
        ...
        var appCache = Path.Combine(FileSystem.AppDataDirectory, "cache");
