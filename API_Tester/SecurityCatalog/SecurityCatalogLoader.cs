...
public static class SecurityCatalogLoader
{
    ...
    public static async Task<SecurityTestCatalog?> LoadAsync()
    {
        _lastLoadError = string.Empty;

        // 1) Preferred: packaged MAUI asset
        try
        {
            await using var stream = await FileSystem.OpenAppPackageFileAsync("security-tests.json");
        ...
        // 2) Fallbacks for desktop/debug scenarios
        var candidates = new[]
        {
